...
void main()
{
    ...
    // barY = step(0.8, barY);
    // barY *= mod(vUv.y * 10.0, 1.0);
    // barY = step(0.4, barY);
    
    // float barX = mod(vUv.x * 10.0, 1.0);
    // barX = step(0.4, barX);
    // barX *= mod(vUv.y * 10.0, 1.0);
    // barX = step(0.8, barX);
    // float strength = barX + barY;

    // Pattern 15
    float barY = mod(vUv.x * 10.0 + 0.2, 1.0);
    barY = step(0.8, barY);
    barY *= mod(vUv.y * 10.0 - 0.2, 1.0);
    barY = step(0.4, barY);
    ...
    // vec2 gridUv = vec2(
    //     floor(vUv.x * 10.0) / 10.0,
    //     floor((vUv.y + vUv.x * 0.5) * 10.0) / 10.0 + 0.3
    // );
    // float strength = random(gridUv);
    ...
    // );
    // float strength = step(abs(distance(wavedUv, vec2(0.5)) - 0.25),0.01);

...
gui.addColor(parameters, 'wallColor')
    .onChange(() => {
        wallMaterial.color = new THREE.Color(parameters.wallColor)
    });
...
gui.addColor(parameters, 'roofColor')
    .onChange(() => {
        roofMaterial.color = new THREE.Color(parameters.roofColor)
    });
house.add(roof)
...
gui.addColor(parameters, 'floorColor')
    .onChange(() => {
        floorMaterial.color = new THREE.Color(parameters.floorColor)
    });
scene.add(floor)
...
const tick = () =>
{
    ...
    ghost3.position.y = Math.sin(ghost3Angle) * (Math.sin(elapsedTime) * 2.5)
